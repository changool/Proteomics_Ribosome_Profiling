...
outputfile1 = open('sitelist.txt', 'w')
...
headlist = ('gi','Gene Symbol','Site Position','Peptide(-10to+10)','Summed PSM')
head = "\t".join(headlist) + '\n'    
...
for x in uniquegilist:
    ...
    for k2,v2 in od.iteritems():
        if v2 > 0:
            if k2-1 <= 10:
                pep10 = pro[:k2-1] + pro[k2-1].lower() + pro[k2:k2-1+11]
            elif k2-1 + 10 > len(pro):
                pep10 = pro[k2-1-10:k2-1] + pro[k2-1].lower() + pro[k2:]
            else:
                pep10 = pro[k2-1-10:k2-1] + pro[k2-1].lower() + pro[k2:k2-1+11]
