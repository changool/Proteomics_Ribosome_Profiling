...
inputfile3 = open (sys.argv[3], 'r')# Concatenated Phosphosite DB from phosphoplus
...
for num3, z in enumerate(inputfile1): # input table: this process is for pTy
     ...
     if table_type == 'PSM':   
        z_ls = z.split('\t')
        ph = z_ls[13].strip() # column for Phosphosite and probability ( 41 for PSM table, 12 for peptide table)
        ph_ls = ph.split(';')
        ratio = z_ls[14]
        dic1 = {}
        score = 0
        gi_ls = z_ls[3].split(';') # list for gi(8 for PSM table, 9 for peptide talbe)
        firstgi = gi_ls[0].strip()
        gilist.append(firstgi)
        pep11 = z_ls[0].strip() # peptide sequence (4 for PSM table, 2 for peptide talbe)
        ...
        if probsum <= 110:
            for zz in ph_ls:
                ...
                if len(zz_ls) < 2:
                    continue
                site = zz_ls[0].strip()
                site1 = re.findall('\w(\d*)\(',site)
                residue = site1[0]
                prob = float(zz_ls[1].strip())
                if site[0] != 'Y':
                    score = score + float(prob)
                else:
                    dic1[site1[0]] = prob
            if len(dic1) == 0:
                continue        
            ...
            if len(dic1) > 0:
                ...
                if float(newprob) >= 75:   
                    newrowlist = (pep11,psm11,firstgi,maxsite,str(newprob),str(ratio))
            ...
            for zz in ph_ls:
                ...
                if len(zz_ls) < 2:
                    continue
                site = zz_ls[0].strip()
                site1 = re.findall('\w(\d*)\(Phos',site)
                residue = site1[0]
                prob = float(zz_ls[1].strip())
                if prob >= 75 and site[0] == 'Y':   
                    newrowlist = (pep11,psm11,firstgi,residue,str(prob),str(ratio))
                    newrow = '\t'.join(newrowlist)
                    newtable.append(newrow)
...
phosphodb = inputfile3.read()
phosphodb = phosphodb.upper()
headlist = ('gi','Gene Symbol','Site Position','Peptide(-7to+7)','Summed PSM','In Phosphosite DB?')
head = "\t".join(headlist) + '\n'    
