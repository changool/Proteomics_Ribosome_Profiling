...
for x in read :
    ...
    if len(x_split) == 4:
    #db order: protein acc|protein gi|RNA acce|RNA seq
       gi = x_split[1]
       gi_rep = gi.replace('gi:','')
       gi_strip = gi_rep.strip()
       rnaacce_strip = x_split[2].strip()
#    lenth = gi_len_seq[1]
       seq_strip = x_split[3].strip()
       rnaacce_rnaseq = rnaacce_strip + "|" + seq_strip
#    gi_len = gi + "|" + lenth
       dic[gi_strip] = rnaacce_rnaseq
...
for linenum, y in enumerate(read1):
