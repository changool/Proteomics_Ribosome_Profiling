...
#TrypR
	   pep001 = pep.replace("RP","YP")
	   pep_rep002 = pep001.replace("R","R|")
#	   pep_rep002 = pep_rep001.replace("D","D|")
	   if pep_rep002[-1] == "|":
	       pep_rep002 = pep_rep002[:-2]
	   pep_enz001 = pep_rep002.split("|")
	   for y001 in pep_enz001:
	      pep_len001 = len(y001)     
	      if pep_len001 >= 6 and pep_len001 <= 40:
		 pep_pos001 = pep.find(y001)
		 for key,value in dic.iteritems():
		     if key >= pep_pos001 and key <= pep_pos001 + pep_len001 -1:
			  dic[key] += 1
	
#GluC
	   pep_rep0001 = pep.replace("E","E|")
	   pep_rep0002 = pep_rep0001.replace("D","D|")
	   if pep_rep0002[-1] == "|":
	       pep_rep0002 = pep_rep0002[:-2]
	   pep_enz0001 = pep_rep0002.split("|")
	   for y0001 in pep_enz0001:
	      pep_len0001 = len(y0001)     
	      if pep_len0001 >= 6 and pep_len0001 <= 40:
		 pep_pos0001 = pep.find(y0001)
		 for key,value in dic.iteritems():
		     if key >= pep_pos0001 and key <= pep_pos0001 + pep_len0001 -1:
			  dic[key] += 1
